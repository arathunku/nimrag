defmodule Nimrag.Credentials do
  ...
  It support MFA by asking user to input code when needed,
  **it's highly recommended that you set up MFA on you Garmin account**.

  Credentials are obtained in a number of ways:
  ...
  * MFA code - by default it's stdin, but you can provide your own function to read it
  ...
  Created OAuth tokesn are stored in `{{config_path}}/oauth1_token.json` and `{{config_path}}/oauth2_token.json`,
  they are valid for 1 hour and are automatically refreshed when needed.
  ...
  @spec new(username :: nil | String.t(), password :: nil | String.t(), get_mfa :: nil | mfa()) ::
          t()
  ...
  @doc """
  Reads OAuth1 token from `~/.config/nimrag/oauth1_token.json`
  ...
  @doc """
  Reads OAuth2 token from `~/.config/nimrag/oauth2_token.json`
  ...
  @doc """
  Writes currently used OAuth1 token to `~/.config/nimrag/oauth1_token.json`
  ...
  @doc """
  Writes currently used OAuth2 token to `~/.config/nimrag/oauth2_token.json`
